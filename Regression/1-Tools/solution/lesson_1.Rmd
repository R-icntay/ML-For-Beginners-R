---
title: 'Machine Learning for Beginners: Regression'
output:
  learnr::tutorial:
    progressive: no
    allow_skip: yes
...
knitr::opts_chunk$set(echo = FALSE) # Prevent knitr chunks from appearing
```
...
Now, let's load these awesome packages and make them available in our current R session.

```{r load_tidy_verse_models, exercise = T, exercise.eval = T, message=F, warning=F, exercise.setup = "setupA"}
...
```{r load_dataset, exercise = T, exercise.eval = T, message=F, warning=F}
# Import the data set
...
```{r select-setup} # This is a setup chunk}
# Import the data set
diabetes <- read_table2(file = "https://www4.stat.ncsu.edu/~boos/var.select/diabetes.rwrite1.txt")

```

```{r select, exercise = T, exercise.eval = T, message=F, warning=F}
...
```{r split-setup} This is a setup chunk}
# Select predictor feature `bmi` and outcome `y`
diabetes_select <- diabetes %>% 
  select(c(bmi, y))
```

```{r split, exercise = T, exercise.eval = T, message=F, warning=F}
set.seed(2056)
...
```{r lm_model_spec, exercise = T, exercise.eval = T, message=F, warning=F}
...
```{r train-setup} # This is a setup chunk}
set.seed(2056)
# Split 67% of the data for training and the rest for tesing
diabetes_split <- diabetes_select %>% 
  initial_split(prop = 0.67)
...
```{r train, exercise = T, exercise.eval = T, message=F, warning=F}
# Build a linear model specification
lm_spec <- linear_reg() %>% 
  set_engine("lm") %>%
  set_mode("regression")
...
```{r test-setup} # This is a setup chunk}
# Build a linear model specification
lm_spec <- linear_reg() %>% 
  set_engine("lm") %>%
  set_mode("regression")
...
```{r test, exercise = T, exercise.eval = T, message=F, warning=F}
...
```{r test_pred-setup} # This is a setup chunk}
# Make predictions for the test set
predictions <- lm_mod %>% 
  predict(new_data = diabetes_test)
```

```{r test_pred, exercise = T, exercise.eval = T, message=F, warning=F}
...
```{r plot_pred, exercise = T, exercise.eval = T, message=F, warning=F}
